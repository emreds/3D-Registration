...
def estimate_pose(mask_lgts: torch.tensor, query_img: torch.tensor,
    ...
    with timer('corr matrix', debug):
        if avg_queries:
            ...
            print(f'Its here in corr matrix debug: {torch.min(corr_matrix_log)}')
        else:
            ...
            print(f'Its not in corr_matrix debug')
        corr_matrix *= mask_prob[:, None]
        print(f'Its corr matrix after: {torch.min(corr_matrix_log)} ')
        if max_pool:
            # max pool spatially
        ...
        print(f'Its corr matrix log after max_pool: {torch.min(corr_matrix_log)} ')
    if poses is None:
    ...
    def batch_score(R: torch.tensor, t: torch.tensor, visualize=False):
        ...
        print(f'This is last corr_matrix_log: {torch.min(corr_matrix_log)}')
        coord_score = corr_matrix_log[u, z_arg]  # (k,)
        print(f'This is coord_score 1:{torch.min(coord_score)}')
        coord_score = torch_scatter.scatter_mean(coord_score, mask_pose_idx, dim_size=n_poses)  # (n_poses,)
        print(f'This is coord_score 2:{torch.min(coord_score)}')
        # handle special case, where no mask pts are in the image
        coord_score_mask = torch.ones(n_poses, dtype=torch.bool, device=device)
        print(f'This is coord_score 3:{torch.min(coord_score)}')
        coord_score_mask[mask_pose_idx] = 0
        print(f'This is coord_score 4:{torch.min(coord_score)}')
        coord_score[coord_score_mask] = -np.inf
        print(f'This is coord_score 5:{torch.min(coord_score)}')
        # normalize by max entropy
        mask_score /= np.log(2)
        coord_score /= np.log(m)
        print(f'This is coord_score 6:{torch.min(coord_score)}')
